...
import { Play, Clock, CheckCircle, XCircle, Plus, Calendar, Timer } from 'lucide-react'
import { getCampaigns, cancelCampaign, subscribeToCampaignUpdates } from '~/queries/campaigns.server'
...
function CampaignsPage() {
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              try {
                ...
                if (['COUNTDOWN_START', 'VIDEO_PLAY', 'VIDEO_END', 'CANCELLED'].includes(parsed.type)) {
                  router.invalidate()
                }
              } catch {
                // Ignore parse errors
  ...
  const activeCampaign = campaigns.find(c => c.status === 'countdown' || c.status === 'playing')
  ...
  return (
    <div className="h-full overflow-auto">
      <div className="container mx-auto p-6 max-w-5xl pb-24 md:pb-6">
        {/* Header */}
            ...
            {activeCampaign ? (
              <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-medium">{activeCampaign.videoName || 'Video'}</p>
                ...
                {scheduledCampaigns.map((campaign) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium">{campaign.videoName || 'Video'}</p>
            ...
            {completedCampaigns.length > 0 ? (
              <div className="space-y-2">
                  ...
                  >
                    ...
                    <span className="text-sm text-muted-foreground">
                      {campaign.completedAt
                        ? format(new Date(campaign.completedAt), 'dd.MM. HH:mm', { locale: sr })
                        : campaign.scheduledAt
                          ? format(new Date(campaign.scheduledAt), 'dd.MM. HH:mm', { locale: sr })
                          : '-'}
                    </span>
                  </div>
                ))}
                {completedCampaigns.length > historyLimit && (
                  ...
                  >
                    Prikaži još ({completedCampaigns.length - historyLimit} preostalo)
                  </Button>
                )}
              </div>
            ) : (
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                {activeCampaign?.id === cancelTarget && ' Video će prestati da se prikazuje.'}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={isCancelling}>Nazad</AlertDialogCancel>
              <AlertDialogAction
