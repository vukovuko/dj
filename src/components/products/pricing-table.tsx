...
export function PricingTable({ products, onChangesUpdate }: PricingTableProps) {
  ...
  const handlePriceChange = (
    ...
    const currentTotal =
      (product.salesCount || 0) + (product.manualSalesAdjustment || 0);
    ...
    setChanges((prev) => ({
      ...
      [productId]: {
        ...
        basePrice:
          field === "basePrice"
            ? value
            : (prev[productId]?.basePrice ?? parseInt(product.basePrice)),
        minPrice:
          field === "minPrice"
            ? value
            : (prev[productId]?.minPrice ?? parseInt(product.minPrice)),
        maxPrice:
          field === "maxPrice"
            ? value
            : (prev[productId]?.maxPrice ?? parseInt(product.maxPrice)),
        totalSalesCount:
              ...
              >
                ...
                <TableCell>
                  <span className="text-sm text-muted-foreground">
                    {Math.round(parseFloat(product.previousPrice))} RSD
                  </span>
                </TableCell>
                <TableCell>
                  <NumberInput
                    ...
                    onValueChange={(value: number | undefined) =>
                      handlePriceChange(product.id, "basePrice", value)
                    }
                  />
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <NumberInput
                      ...
                      onValueChange={(value: number | undefined) =>
                        handlePriceChange(product.id, "totalSalesCount", value)
                      }
                    />
