...
async function waitForPostgres(maxRetries = 30) {
  for (let i = 1; i <= maxRetries; i++) {
    try {
      execSync(
        'docker exec dj-postgres-dev pg_isready -U djuser -d djcafe',
        { stdio: "pipe" }
      )
      ...
      process.stdout.write(`⏳ Waiting for PostgreSQL... (${i}/${maxRetries})\r`)
...
async function main() {
  ...
  const dev = spawn("npx", ["vite", "dev"], { stdio: "inherit", shell: true, cwd: process.cwd() })
